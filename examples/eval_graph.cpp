...
int main(int argc, char** argv) {
  ...
  auto eval_publisher_socket =
      std::make_shared<zmq::socket_t>(zmq_context, ZMQ_PUB);
  eval_publisher_socket->bind("inproc://eval_node");
  auto eval_publisher_synchronize_socket =
      std::make_shared<zmq::socket_t>(zmq_context, ZMQ_REP);
  eval_publisher_synchronize_socket->bind("inproc://eval_node_sync");
  std::shared_ptr<Consumer> eval_consumer =
      std::make_shared<PublisherConsumer>(
          TransportUtils::Publisher(eval_publisher_socket,
  ...
  auto sum_expr = gandiva::TreeExprBuilder::MakeExpression(
      "add", {field0, field1}, field_sum);
  auto subtract_expr = gandiva::TreeExprBuilder::MakeExpression(
      "subtract", {field0, field1}, field_diff);
  ...
  std::shared_ptr<Consumer> aggregate_consumer =
