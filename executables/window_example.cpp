...
int main(int argc, char** argv) {
  ...
  auto window_subscriber_synchronize_socket = std::make_shared<zmq::socket_t>(zmq_context, ZMQ_REQ);
  window_subscriber_synchronize_socket->connect("inproc://input_node_sync");
  std::shared_ptr<Producer> window_producer = std::make_shared<SubscriberProducer>(
