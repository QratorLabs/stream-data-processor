...
class RequestHandler {
 public:
  explicit RequestHandler(const std::shared_ptr<IUDFAgent>& agent) : agent_(agent) {
  ...
  [[nodiscard]] virtual agent::Response init(const agent::InitRequest& init_request) = 0;
  [[nodiscard]] virtual agent::Response snapshot() const = 0;
  [[nodiscard]] virtual agent::Response restore(const agent::RestoreRequest& restore_request) = 0;
