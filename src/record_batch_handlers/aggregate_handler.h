...
class AggregateHandler : public RecordBatchHandler {
 public:
  enum AggregateFunctionEnumType {
    kFirst,
    kLast,
    kMax,
    kMin,
  ...
  struct AggregateOptions {
    std::unordered_map<std::string, std::vector<AggregateCase>> aggregate_columns;
 ...
 private:
  ...
  static const std::unordered_map<AggregateFunctionEnumType, std::shared_ptr<AggregateFunction>> TYPES_TO_FUNCTIONS;
};
