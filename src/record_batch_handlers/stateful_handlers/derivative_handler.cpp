...
arrow::Result<arrow::RecordBatchVector> DerivativeHandler::handle(
  ...
  if (buffered_times_.empty()) {
    ARROW_ASSIGN_OR_RAISE(left_bound_time, getPositionTime(0, *time_column));
  } else {
  ...
  ARROW_ASSIGN_OR_RAISE(right_bound_time,
                        getPositionTime(right_bound_row_id, *time_column));

  auto total_rows = time_column->length();
  while (derivative_row_id < total_rows) {
    ARROW_ASSIGN_OR_RAISE(derivative_time,
                          getPositionTime(derivative_row_id, *time_column));
        ...
        ARROW_RETURN_NOT_OK(
            derivative_columns_builders[result_column_name].Append(
                derivative_calculator_->calculateDerivative(
                    ...
                    buffered_times_, derivative_time,
                    derivative_case.order)));
...
arrow::Result<double> DerivativeHandler::getPositionValue(
  ...
  ARROW_ASSIGN_OR_RAISE(auto value_scalar, value_column.GetScalar(id));
